...
conn_uri = os.getenv('TNT_CONN_URI', 'tcp://bee:3301')
...
if image.startswith('php:'):
    ...
    if 'pecl' == client:
        run_cmds.append('git clone https://github.com/bee/bee-php.git /usr/src/php/ext/bee')
        if image.startswith('php:7'):
            run_cmds.append('git --git-dir=/usr/src/php/ext/bee/.git --work-tree=/usr/src/php/ext/bee checkout php7-v2')

        run_cmds.append('echo bee >> /usr/src/php-available-exts && docker-php-ext-install bee')
